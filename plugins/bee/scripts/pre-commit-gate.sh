        ...
        case "$STACK_LINTER" in
            pint)
                if [ -f "$WORK_DIR/vendor/bin/pint" ]; then
                    (cd "$WORK_DIR" && vendor/bin/pint --test 2>&1) || LINT_FAILED=1
                fi
                ;;
            eslint)
                if [ -f "$WORK_DIR/node_modules/.bin/eslint" ]; then
                    (cd "$WORK_DIR" && npx eslint . 2>&1) || LINT_FAILED=1
                fi
                ;;
            prettier)
                if [ -f "$WORK_DIR/node_modules/.bin/prettier" ]; then
                    (cd "$WORK_DIR" && npx prettier --check . 2>&1) || LINT_FAILED=1
                fi
                ;;
            biome)
                if [ -f "$WORK_DIR/node_modules/.bin/biome" ]; then
                    (cd "$WORK_DIR" && npx biome check . 2>&1) || LINT_FAILED=1
                fi
                ;;
        esac
    fi
        ...
        case "$STACK_RUNNER" in
            pest)
                if [ -f "$WORK_DIR/vendor/bin/pest" ]; then
                    (cd "$WORK_DIR" && php artisan test --parallel 2>&1) || TEST_FAILED=1
                fi
                ;;
            vitest)
                if [ -f "$WORK_DIR/node_modules/.bin/vitest" ]; then
                    (cd "$WORK_DIR" && npx vitest run 2>&1) || TEST_FAILED=1
                fi
                ;;
            jest)
                if [ -f "$WORK_DIR/node_modules/.bin/jest" ]; then
                    (cd "$WORK_DIR" && npx jest 2>&1) || TEST_FAILED=1
                fi
                ;;
        esac
    fi
...
if [ "$LINT_FAILED" = "1" ]; then
    echo '{"decision": "block", "reason": "Pre-commit gate: linter errors found. Fix them before committing."}' >&2
    exit 2
fi

if [ "$TEST_FAILED" = "1" ]; then
    echo '{"decision": "block", "reason": "Pre-commit gate: tests failing. Fix them before committing."}' >&2
    exit 2
fi
